...
def parse_incapto(response):
    # Capturar todos los productos en la lista de productos
    products = response.css('.card-item')
    ...
    for idx, product in enumerate(products, 1):
        product_name = product.css('h3.card-text::text').get().strip()
...
def parse_product_details(response):
    ...
    print(f"Producto: {product_name} | Origen: {product_origin} | Precio 500g: {price_500} | Precio 1kg: {price_1_kilo}")
    ...
    if product_origin:
        yield {
            ...
            'price_250': None,
            'price_500': format_price(price_500) if price_500 else None,
